    ...
    def action_custom_revision(self):
        ...
        if self._context and self._context.get('active_ids', False):
            for custom in self.env['operation.custom'].browse(
                ...
                rev_name = 'Revision-' + str(custom.revision_count + 1) + \
                    '/Custom/' + str(custom.operation_id.name)
                revision = rev_obj.create({
                    'name': rev_name,
                    'operation_id': custom.operation_id and
                    custom.operation_id.id or False,
                    'custom_id': custom.id or False,
                    'agent_id': custom.agent_id and
                    custom.agent_id.id or False,
